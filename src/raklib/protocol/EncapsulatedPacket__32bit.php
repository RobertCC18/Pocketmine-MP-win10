...
class EncapsulatedPacket{
	...
	public static function fromBinary($binary, $internal = \false, &$offset = \null){
		...
		if($internal){
			$length = (\unpack("N", \substr($binary, 1, 4))[1]);
			$packet->identifierACK = (\unpack("N", \substr($binary, 5, 4))[1]);
			$offset = 9;
		}else{
		...
		if($hasSplit){
			$packet->splitCount = (\unpack("N", \substr($binary, $offset, 4))[1]);
			$offset += 4;
			$packet->splitID = (\unpack("n", \substr($binary, $offset, 2))[1]);
			$offset += 2;
			$packet->splitIndex = (\unpack("N", \substr($binary, $offset, 4))[1]);
			$offset += 4;
		}
