...
class SessionManager{
		...
		if($buffer !== \null){
			...
			if($len > 0){
				...
				if($pid === UNCONNECTED_PING::$ID){
					...
					//ignored
				}elseif(($packet = $this->getPacketFromPool($pid)) !== \null){
					$packet->buffer = $buffer;
	...
	protected function streamOpen(Session $session){
		...
		$buffer = \chr(RakLib::PACKET_OPEN_SESSION) . \chr(\strlen($identifier)) . $identifier . \chr(\strlen($session->getAddress())) . $session->getAddress() . (\pack("n", $session->getPort())) . (\pack("NN", $session->getID() >> 32, $session->getID() & 0xFFFFFFFF));
		$this->server->pushThreadToMainPacket($buffer);
	}
