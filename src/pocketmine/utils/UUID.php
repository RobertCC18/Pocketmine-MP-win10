...
class UUID{
	...
	public function __construct($part1 = 0, $part2 = 0, $part3 = 0, $part4 = 0, $version = \null){
		$this->parts[0] = (int) $part1;
		$this->parts[1] = (int) $part2;
		$this->parts[2] = (int) $part3;
		$this->parts[3] = (int) $part4;

		$this->version = $version === \null ? ($this->parts[1] & 0xf000) >> 12 : (int) $version;
	}
	 ...
	 * @return UUID
	 */
	public static function fromString($uuid, $version = \null){
		return self::fromBinary(\hex2bin(\str_replace("-", "", \trim($uuid))), $version);
	}
	...
	public static function fromBinary($uuid, $version = \null){
		...
		return new UUID((\PHP_INT_SIZE === 8 ? \unpack("N", \substr($uuid, 0, 4))[1] << 32 >> 32 : \unpack("N", \substr($uuid, 0, 4))[1]), (\PHP_INT_SIZE === 8 ? \unpack("N", \substr($uuid, 4, 4))[1] << 32 >> 32 : \unpack("N", \substr($uuid, 4, 4))[1]), (\PHP_INT_SIZE === 8 ? \unpack("N", \substr($uuid, 8, 4))[1] << 32 >> 32 : \unpack("N", \substr($uuid, 8, 4))[1]), (\PHP_INT_SIZE === 8 ? \unpack("N", \substr($uuid, 12, 4))[1] << 32 >> 32 : \unpack("N", \substr($uuid, 12, 4))[1]), $version);
	}
	...
	public static function fromData(...$data){
		$hash = \hash("md5", \implode($data), \true);

		return self::fromBinary($hash, 3);
	}

	public static function fromRandom(){
		return self::fromData((\pack("N", \time())), (\pack("n", \getmypid())), (\pack("n", \getmyuid())), (\pack("N", \mt_rand(-0x7fffffff, 0x7fffffff))), (\pack("N", \mt_rand(-0x7fffffff, 0x7fffffff))));
	}
