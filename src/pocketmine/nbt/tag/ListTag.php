...
class ListTag extends NamedTag implements \ArrayAccess, \Countable{
	...
	public function read(NBT $nbt, bool $network = \false){
		...
		$size = ($network === \true ? Binary::readVarInt($nbt) : ($nbt->endianness === 1 ? (\PHP_INT_SIZE === 8 ? \unpack("N", $nbt->get(4))[1] << 32 >> 32 : \unpack("N", $nbt->get(4))[1]) : (\PHP_INT_SIZE === 8 ? \unpack("V", $nbt->get(4))[1] << 32 >> 32 : \unpack("V", $nbt->get(4))[1])));
	...
	public function write(NBT $nbt, bool $network = \false){
		...
		($nbt->buffer .=  $network === \true ? Binary::writeVarInt(\count($tags)) : ($nbt->endianness === 1 ? (\pack("N", \count($tags))) : (\pack("V", \count($tags)))));
		foreach($tags as $tag){
