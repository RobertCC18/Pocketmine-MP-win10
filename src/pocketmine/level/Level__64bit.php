...
class Level implements ChunkManager, Metadatable{
	...
	public function registerChunkLoader(ChunkLoader $loader, int $chunkX, int $chunkZ, bool $autoLoad = \true){
		$hash = $loader->getLoaderId();

		if(!isset($this->chunkLoaders[$index = ((($chunkX) & 0xFFFFFFFF) << 32) | (( $chunkZ) & 0xFFFFFFFF)])){
	...
	public function unregisterChunkLoader(ChunkLoader $loader, int $chunkX, int $chunkZ){
		if(isset($this->chunkLoaders[$index = ((($chunkX) & 0xFFFFFFFF) << 32) | (( $chunkZ) & 0xFFFFFFFF)][$hash = $loader->getLoaderId()])){
	...
	public function doTick(int $currentTick){
		...
		while($this->updateQueue->count() > 0 and $this->updateQueue->current()["priority"] <= $currentTick){
			...
			unset($this->updateQueueIndex[((($block->x) & 0xFFFFFFF) << 35) | ((( $block->y) & 0x7f) << 28) | (( $block->z) & 0xFFFFFFF)]);
	...
	public function sendBlockExtraData(int $x, int $y, int $z, int $id, int $data, array $targets = \null){
		...
		Server::broadcastPacket($targets === \null ? $this->getChunkPlayers($x >> 4, $z >> 4) : $targets, $pk);
	}
	...
	public function scheduleUpdate(Vector3 $pos, int $delay){
		if(isset($this->updateQueueIndex[$index = ((($pos->x) & 0xFFFFFFF) << 35) | ((( $pos->y) & 0x7f) << 28) | (( $pos->z) & 0xFFFFFFF)]) and $this->updateQueueIndex[$index] <= $delay){
			return;
		}
	 ...
	 */
	public function getCollisionCubes(Entity $entity, AxisAlignedBB $bb, bool $entities = \true) : array{
		$minX = Math::floorFloat($bb->minX);
		$minY = Math::floorFloat($bb->minY);
		$minZ = Math::floorFloat($bb->minZ);
		$maxX = Math::ceilFloat($bb->maxX);
	...
	*/
	 ...
	 * @return Block
	 */
	public function getBlock(Vector3 $pos, $cached = \true) : Block{
		$pos = $pos->floor();
		$index = ((($pos->x) & 0xFFFFFFF) << 35) | ((( $pos->y) & 0x7f) << 28) | (( $pos->z) & 0xFFFFFFF);
		if($cached and isset($this->blockCache[$index])){
			return $this->blockCache[$index];
		}elseif($pos->y >= 0 and $pos->y < $this->provider->getWorldHeight() and isset($this->chunks[$chunkIndex = ((($pos->x >> 4) & 0xFFFFFFFF) << 32) | (( $pos->z >> 4) & 0xFFFFFFFF)])){
	...
	public function updateBlockLight(int $x, int $y, int $z){
		...
		if($oldLevel !== $newLevel){
			...
			if($newLevel < $oldLevel){
				$removalVisited[((($x) & 0xFFFFFFF) << 35) | ((( $y) & 0x7f) << 28) | (( $z) & 0xFFFFFFF)] = \true;
				$lightRemovalQueue->enqueue([new Vector3($x, $y, $z), $oldLevel]);
			}else{
				$visited[((($x) & 0xFFFFFFF) << 35) | ((( $y) & 0x7f) << 28) | (( $z) & 0xFFFFFFF)] = \true;
	...
	private function computeRemoveBlockLight(int $x, int $y, int $z, int $currentLight, \SplQueue $queue, \SplQueue $spreadQueue, array &$visited, array &$spreadVisited){
		...
		if($current !== 0 and $current < $currentLight){
			...
			if(!isset($visited[$index = ((($x) & 0xFFFFFFF) << 35) | ((( $y) & 0x7f) << 28) | (( $z) & 0xFFFFFFF)])){
				$visited[$index] = \true;
				if($current > 1){
			...
			if(!isset($spreadVisited[$index = ((($x) & 0xFFFFFFF) << 35) | ((( $y) & 0x7f) << 28) | (( $z) & 0xFFFFFFF)])){
	 ...
	 */
	public function setBlock(Vector3 $pos, Block $block, bool $direct = \false, bool $update = \true) : bool{
		$pos = $pos->floor();
		if($pos->y < 0 or $pos->y >= $this->provider->getWorldHeight()){
			return \false;
		}

		if($this->getChunk($pos->x >> 4, $pos->z >> 4, \true)->setBlock($pos->x & 0x0f, $pos->y & Level::Y_MASK, $pos->z & 0x0f, $block->getId(), $block->getDamage())){
			if(!($pos instanceof Position)){
				$pos = $this->temporalPosition->setComponents($pos->x, $pos->y, $pos->z);
			}

			$block->position($pos);
			unset($this->blockCache[((($pos->x) & 0xFFFFFFF) << 35) | ((( $pos->y) & 0x7f) << 28) | (( $pos->z) & 0xFFFFFFF)]);

			$index = ((($pos->x >> 4) & 0xFFFFFFFF) << 32) | (( $pos->z >> 4) & 0xFFFFFFFF);
			...
			if($direct === \true){
				...
				$this->changedBlocks[$index][((($block->x) & 0xFFFFFFF) << 35) | ((( $block->y) & 0x7f) << 28) | (( $block->z) & 0xFFFFFFF)] = clone $block;
			}
	...
	public function dropItem(Vector3 $source, Item $item, Vector3 $motion = \null, int $delay = 10){
		...
		if($item->getId() > 0 and $item->getCount() > 0){
			$itemEntity = Entity::createEntity("Item", $this->getChunk($source->getX() >> 4, $source->getZ() >> 4, \true), new CompoundTag("", [
				"Pos" => new ListTag("Pos", [
		...
		if($tag instanceof ListTag){
			...
			foreach($tag as $v){
				if($v instanceof StringTag){
					...
					if($entry->getId() > 0 and $entry->getBlock() !== \null and $entry->getBlock()->getId() === $target->getId()){
						$canBreak = \true;
						break;
					}
				}
	 ...
	 * @return bool
	 */
	public function useItemOn(Vector3 $vector, Item &$item, int $face, float $fx = 0.0, float $fy = 0.0, float $fz = 0.0, Player $player = \null) : bool{
		$target = $this->getBlock($vector);
		...
		if($player !== \null){
			...
			if(!$ev->isCancelled()){
				...
				if(!$player->isSneaking() and $target->canBeActivated() === \true and $target->onActivate($item, $player) === \true){
					return \true;
				}
		...
		if(!($block->canBeReplaced() === \true or ($hand->getId() === Item::WOOD_SLAB and $block->getId() === Item::WOOD_SLAB) or ($hand->getId() === Item::SLAB and $block->getId() === Item::SLAB))){
			return \false;
		}
	...
	public function getBlockIdAt(int $x, int $y, int $z) : int{
		return $this->getChunk($x >> 4, $z >> 4, \true)->getBlockId($x & 0x0f, $y & Level::Y_MASK, $z & 0x0f);
	}
	...
	public function setBlockIdAt(int $x, int $y, int $z, int $id){
		unset($this->blockCache[((($x) & 0xFFFFFFF) << 35) | ((( $y) & 0x7f) << 28) | (( $z) & 0xFFFFFFF)]);
		$this->getChunk($x >> 4, $z >> 4, \true)->setBlockId($x & 0x0f, $y & Level::Y_MASK, $z & 0x0f, $id & 0xff);

		if(!isset($this->changedBlocks[$index = ((($x >> 4) & 0xFFFFFFFF) << 32) | (( $z >> 4) & 0xFFFFFFFF)])){
			$this->changedBlocks[$index] = [];
		}
		$this->changedBlocks[$index][((($x) & 0xFFFFFFF) << 35) | ((( $y) & 0x7f) << 28) | (( $z) & 0xFFFFFFF)] = $v = new Vector3($x, $y, $z);
	...
	public function getBlockExtraDataAt(int $x, int $y, int $z) : int{
		return $this->getChunk($x >> 4, $z >> 4, \true)->getBlockExtraData($x & 0x0f, $y & Level::Y_MASK, $z & 0x0f);
	}
	...
	public function setBlockExtraDataAt(int $x, int $y, int $z, int $id, int $data){
		$this->getChunk($x >> 4, $z >> 4, \true)->setBlockExtraData($x & 0x0f, $y & Level::Y_MASK, $z & 0x0f, ($data << 8) | $id);
	...
	public function getBlockDataAt(int $x, int $y, int $z) : int{
		return $this->getChunk($x >> 4, $z >> 4, \true)->getBlockData($x & 0x0f, $y & Level::Y_MASK, $z & 0x0f);
	}
	...
	public function setBlockDataAt(int $x, int $y, int $z, int $data){
		unset($this->blockCache[((($x) & 0xFFFFFFF) << 35) | ((( $y) & 0x7f) << 28) | (( $z) & 0xFFFFFFF)]);
		$this->getChunk($x >> 4, $z >> 4, \true)->setBlockData($x & 0x0f, $y & Level::Y_MASK, $z & 0x0f, $data & 0x0f);
	...
	public function getBlockSkyLightAt(int $x, int $y, int $z) : int{
		return $this->getChunk($x >> 4, $z >> 4, \true)->getBlockSkyLight($x & 0x0f, $y & Level::Y_MASK, $z & 0x0f);
	}
	...
	public function setBlockSkyLightAt(int $x, int $y, int $z, int $level){
		$this->getChunk($x >> 4, $z >> 4, \true)->setBlockSkyLight($x & 0x0f, $y & Level::Y_MASK, $z & 0x0f, $level & 0x0f);
	}
	...
	public function getBlockLightAt(int $x, int $y, int $z) : int{
		return $this->getChunk($x >> 4, $z >> 4, \true)->getBlockLight($x & 0x0f, $y & Level::Y_MASK, $z & 0x0f);
	}
	...
	public function setBlockLightAt(int $x, int $y, int $z, int $level){
		$this->getChunk($x >> 4, $z >> 4, \true)->setBlockLight($x & 0x0f, $y & Level::Y_MASK, $z & 0x0f, $level & 0x0f);
	}
	...
	public function isChunkLoaded(int $x, int $z) : bool{
		return isset($this->chunks[((($x) & 0xFFFFFFFF) << 32) | (( $z) & 0xFFFFFFFF)]) or $this->provider->isChunkLoaded($x, $z);
	}
	...
	private function sendChunkFromCache(int $x, int $z){
		if(isset($this->chunkSendTasks[$index = ((($x) & 0xFFFFFFFF) << 32) | (( $z) & 0xFFFFFFFF)])){
		...
		if(\count($this->chunkSendQueue) > 0){
			...
			foreach($this->chunkSendQueue as $index => $players){
				if(isset($this->chunkSendTasks[$index])){
				 ...
				 $x = ($index >> 32) << 32 >> 32;  $z = ($index & 0xFFFFFFFF) << 32 >> 32;;
	 ...
	 */
	public function loadChunk(int $x, int $z, bool $generate = \true) : bool{
		if(isset($this->chunks[$index = ((($x) & 0xFFFFFFFF) << 32) | (( $z) & 0xFFFFFFFF)])){
			return \true;
		}
	...
	private function queueUnloadChunk(int $x, int $z){
		$this->unloadQueue[$index = ((($x) & 0xFFFFFFFF) << 32) | (( $z) & 0xFFFFFFFF)] = \microtime(\true);
		unset($this->chunkTickList[$index]);
	}

	public function unloadChunkRequest(int $x, int $z, bool $safe = \true){
	...
	public function unloadChunk(int $x, int $z, bool $safe = \true, bool $trySave = \true) : bool{
		if(($safe === \true and $this->isChunkInUse($x, $z))){
			return \false;
		}

		if(!(isset($this->chunks[((($x) & 0xFFFFFFFF) << 32) | ((  $z) & 0xFFFFFFFF)]) or $this->provider->isChunkLoaded($x,  $z))){
			return \true;
		}
		...
		if(!$chunk->isPopulated()){
			...
			if($populate){
				if(!isset($this->chunkPopulationQueue[$index])){
					...
					for($xx = -1; $xx <= 1; ++$xx){
						for($zz = -1; $zz <= 1; ++$zz){
							$this->chunkPopulationLock[((($x + $xx) & 0xFFFFFFFF) << 32) | (( $z + $zz) & 0xFFFFFFFF)] = \true;
						}
					}
	...
	public function generateChunk(int $x, int $z, bool $force = \false){
		...
		if(!isset($this->chunkGenerationQueue[$index = ((($x) & 0xFFFFFFFF) << 32) | (( $z) & 0xFFFFFFFF)])){
	...
	public function addEntityMotion(int $chunkX, int $chunkZ, int $entityId, float $x, float $y, float $z){
		if(!isset($this->motionToSend[$index = ((($chunkX) & 0xFFFFFFFF) << 32) | (( $chunkZ) & 0xFFFFFFFF)])){
			$this->motionToSend[$index] = [];
		}
		$this->motionToSend[$index][$entityId] = [$entityId, $x, $y, $z];
	}

	public function addEntityMovement(int $chunkX, int $chunkZ, int $entityId, float $x, float $y, float $z, float $yaw, float $pitch, $headYaw = \null){
		if(!isset($this->moveToSend[$index = ((($chunkX) & 0xFFFFFFFF) << 32) | (( $chunkZ) & 0xFFFFFFFF)])){
