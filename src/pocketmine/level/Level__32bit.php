...
class Level implements ChunkManager, Metadatable{
	...
	public function getChunkPlayers(int $chunkX, int $chunkZ) : array{
		return $this->playerLoaders[(($chunkX) . ":" . ( $chunkZ))] ?? [];
	}
	...
	public function getChunkLoaders(int $chunkX, int $chunkZ) : array{
		return $this->chunkLoaders[(($chunkX) . ":" . ( $chunkZ))] ?? [];
	}

	public function addChunkPacket(int $chunkX, int $chunkZ, DataPacket $packet){
		if(!isset($this->chunkPackets[$index = (($chunkX) . ":" . ( $chunkZ))])){
	...
	public function registerChunkLoader(ChunkLoader $loader, int $chunkX, int $chunkZ, bool $autoLoad = \true){
		...
		if(!isset($this->chunkLoaders[$index = (($chunkX) . ":" . ( $chunkZ))])){
	...
	public function unregisterChunkLoader(ChunkLoader $loader, int $chunkX, int $chunkZ){
		if(isset($this->chunkLoaders[$index = (($chunkX) . ":" . ( $chunkZ))][$hash = $loader->getLoaderId()])){
	...
	public function sendBlocks(array $target, array $blocks, $flags = UpdateBlockPacket::FLAG_NONE, bool $optimizeRebuilds = \false){
		if($optimizeRebuilds){
			...
			foreach($blocks as $b){
				...
				$first = \false;
				if(!isset($chunks[$index = (($b->x >> 4) . ":" . ( $b->z >> 4))])){
					$chunks[$index] = \true;
					$first = \true;
				}
	...
	public function clearChunkCache(int $chunkX, int $chunkZ){
		unset($this->chunkCache[(($chunkX) . ":" . ( $chunkZ))]);
	}
	...
	public function scheduleUpdate(Vector3 $pos, int $delay){
		if(isset($this->updateQueueIndex[$index = (($pos->x) . ":" . ( $pos->y) .":". ( $pos->z))]) and $this->updateQueueIndex[$index] <= $delay){
			return;
		}
	...
	*/
	...
	public function getBlock(Vector3 $pos, $cached = \true) : Block{
		...
		}elseif($pos->y >= 0 and $pos->y < $this->provider->getWorldHeight() and isset($this->chunks[$chunkIndex = (($pos->x >> 4) . ":" . ( $pos->z >> 4))])){
	...
	public function setBlock(Vector3 $pos, Block $block, bool $direct = \false, bool $update = \true) : bool{
		...
		if($this->getChunk($pos->x >> 4, $pos->z >> 4, \true)->setBlock($pos->x & 0x0f, $pos->y & Level::Y_MASK, $pos->z & 0x0f, $block->getId(), $block->getDamage())){
			...
			if($direct === \true){
				...
				$this->changedBlocks[$index][(($block->x) . ":" . ( $block->y) .":". ( $block->z))] = clone $block;
			}
	...
	public function setBlockIdAt(int $x, int $y, int $z, int $id){
		...
		if(!isset($this->changedBlocks[$index = (($x >> 4) . ":" . ( $z >> 4))])){
			$this->changedBlocks[$index] = [];
		}
		$this->changedBlocks[$index][(($x) . ":" . ( $y) .":". ( $z))] = $v = new Vector3($x, $y, $z);
	...
	public function generateChunkCallback(int $x, int $z, Chunk $chunk){
		...
		if(isset($this->chunkPopulationQueue[$index = (($x) . ":" . ( $z))])){
			$oldChunk = $this->getChunk($x, $z, \false);
			for($xx = -1; $xx <= 1; ++$xx){
				for($zz = -1; $zz <= 1; ++$zz){
					unset($this->chunkPopulationLock[(($x + $xx) . ":" . ( $z + $zz))]);
				}
			}
	...
	private function sendChunkFromCache(int $x, int $z){
		if(isset($this->chunkSendTasks[$index = (($x) . ":" . ( $z))])){
	...
	private function processChunkRequest(){
		if(\count($this->chunkSendQueue) > 0){
			...
			foreach($this->chunkSendQueue as $index => $players){
				...
				list( $x,  $z) = \explode(":", $index);  $x = (int)  $x;  $z = (int)  $z;;
	...
	public function isChunkInUse(int $x, int $z) : bool{
		return isset($this->chunkLoaders[$index = (($x) . ":" . ( $z))]) and \count($this->chunkLoaders[$index]) > 0;
	}
	...
	private function queueUnloadChunk(int $x, int $z){
		$this->unloadQueue[$index = (($x) . ":" . ( $z))] = \microtime(\true);
		unset($this->chunkTickList[$index]);
	}
	...
	public function unloadChunk(int $x, int $z, bool $safe = \true, bool $trySave = \true) : bool{
		...
		if(!(isset($this->chunks[(($x) . ":" . (  $z))]) or $this->provider->isChunkLoaded($x,  $z))){
			return \true;
		}
	...
	public function populateChunk(int $x, int $z, bool $force = \false) : bool{
		if(isset($this->chunkPopulationQueue[$index = (($x) . ":" . ( $z))]) or (\count($this->chunkPopulationQueue) >= $this->chunkPopulationQueueSize and !$force)){
			return \false;
		}
		...
		if(!$chunk->isPopulated()){
			...
			for($xx = -1; $xx <= 1; ++$xx){
				for($zz = -1; $zz <= 1; ++$zz){
					if(isset($this->chunkPopulationLock[(($x + $xx) . ":" . ( $z + $zz))])){
						$populate = \false;
						break;
					}
				}
			...
			if($populate){
				if(!isset($this->chunkPopulationQueue[$index])){
					...
					for($xx = -1; $xx <= 1; ++$xx){
						for($zz = -1; $zz <= 1; ++$zz){
							$this->chunkPopulationLock[(($x + $xx) . ":" . ( $z + $zz))] = \true;
						}
					}
	...
	public function generateChunk(int $x, int $z, bool $force = \false){
		...
		if(!isset($this->chunkGenerationQueue[$index = (($x) . ":" . ( $z))])){
	...
	public function addEntityMotion(int $chunkX, int $chunkZ, int $entityId, float $x, float $y, float $z){
		if(!isset($this->motionToSend[$index = (($chunkX) . ":" . ( $chunkZ))])){
	...
	public function addEntityMovement(int $chunkX, int $chunkZ, int $entityId, float $x, float $y, float $z, float $yaw, float $pitch, $headYaw = \null){
		if(!isset($this->moveToSend[$index = (($chunkX) . ":" . ( $chunkZ))])){
