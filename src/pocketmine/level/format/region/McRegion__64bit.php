...
class McRegion extends BaseLevelProvider{
	...
	public function unloadChunk(int $chunkX, int $chunkZ, bool $safe = \true) : bool{
		$chunk = $this->chunks[$index = ((($chunkX) & 0xFFFFFFFF) << 32) | (( $chunkZ) & 0xFFFFFFFF)] ?? \null;
	...
	public function isChunkLoaded(int $chunkX, int $chunkZ) : bool{
		return isset($this->chunks[((($chunkX) & 0xFFFFFFFF) << 32) | (( $chunkZ) & 0xFFFFFFFF)]);
	}
	...
	protected function getRegion(int $x, int $z){
		return $this->regions[((($x) & 0xFFFFFFFF) << 32) | (( $z) & 0xFFFFFFFF)] ?? \null;
	}
