...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer{
	...
	public function sendChunk($x, $z, $payload){
		...
		$this->usedChunks[((($x) & 0xFFFFFFFF) << 32) | (( $z) & 0xFFFFFFFF)] = \true;
		$this->chunkLoadCount++;
	...
	protected function orderChunks(){
		...
		for($i = 0; $i < $viewDistance; ++$i){
			...
			if(!isset($this->usedChunks[$index = ((($chunkX) & 0xFFFFFFFF) << 32) | (( $chunkZ) & 0xFFFFFFFF)]) or $this->usedChunks[$index] === \false){
				$newOrder[$index] = \true;
			}
	...
	public function sleepOn(Vector3 $pos){
		...
		$this->setDataFlag(self::DATA_PLAYER_FLAGS, self::DATA_PLAYER_FLAG_SLEEP, \true, self::DATA_TYPE_BYTE);
	...
	protected function processLogin(){
		...
		$nbt->lastPlayed = new LongTag("lastPlayed", \floor(\microtime(\true) * 1000));
		...
		$this->sendAttributes(\true);
		$this->setNameTagVisible(\true);
		$this->setNameTagAlwaysVisible(\true);
		...
		switch($packet::NETWORK_ID){
			...
			case ProtocolInfo::INTERACT_PACKET:
				...
				switch($packet->action){
					case InteractPacket::ACTION_LEFT_CLICK: //Attack
						if($target instanceof Player and $this->server->getConfigBoolean("pvp", \true) === \false){
							$cancelled = \true;
						}
				...
				if($recipe instanceof ShapedRecipe){
					for($x = 0; $x < 3 and $canCraft; ++$x){
						for($y = 0; $y < 3; ++$y){
							...
							if($item->getCount() > 0){
								if($ingredient === \null or !$ingredient->deepEquals($item, !$ingredient->hasAnyDamageValue(), $ingredient->hasCompoundTag())){
									$canCraft = \false;
									break;
								}
							}
			...
			case ProtocolInfo::BLOCK_ENTITY_DATA_PACKET:
				...
				if($t instanceof Spawnable){
					...
					$nbt->read($packet->namedtag, \false, \true);
					$nbt = $nbt->getData();
			...
			case ProtocolInfo::ITEM_FRAME_DROP_ITEM_PACKET:
				...
				if(($tile = $this->level->getTile($this->temporalVector->setComponents($packet->x, $packet->y, $packet->z))) instanceof ItemFrame){
					if(!$tile->getItem()->equals($packet->item) and !$this->isCreative(\true)){
						$tile->spawnTo($this);
						break;
					}
	...
	protected function checkTeleportPosition(){
		if($this->teleportPosition !== \null){
			...
			for($X = -1; $X <= 1; ++$X){
				for($Z = -1; $Z <= 1; ++$Z){
					if(!isset($this->usedChunks[$index = ((($chunkX + $X) & 0xFFFFFFFF) << 32) | (( $chunkZ + $Z) & 0xFFFFFFFF)]) or $this->usedChunks[$index] === \false){
						return \false;
					}
				}
			}
