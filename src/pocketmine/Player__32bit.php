...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer{
	...
	protected function orderChunks(){
		...
		for($i = 0; $i < $viewDistance; ++$i){
			...
			if(!isset($this->usedChunks[$index = (($chunkX) . ":" . ( $chunkZ))]) or $this->usedChunks[$index] === \false){
				$newOrder[$index] = \true;
			}
	...
	protected function checkTeleportPosition(){
		if($this->teleportPosition !== \null){
			...
			for($X = -1; $X <= 1; ++$X){
				for($Z = -1; $Z <= 1; ++$Z){
					if(!isset($this->usedChunks[$index = (($chunkX + $X) . ":" . ( $chunkZ + $Z))]) or $this->usedChunks[$index] === \false){
						return \false;
					}
				}
			}
